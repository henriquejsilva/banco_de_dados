...
left join pedidos on clientes.id = clientes.id
 ...
 select nome, count(id) from clientes
...
inner join clientes on pedidos.cliente_id = clientes.id;
  ...
  select max(valor) as 'maior valor' from pizzas where cliente_id;
  ...
  -- Qual o pedido mais caro, exibir o nome do cliente e o valor do pedido
  select max(valor) as 'maior valor' from pizzas;
